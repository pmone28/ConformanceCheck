...
def export_rule_conformance_csv(results_df):
    ...
    export_data = {
        "id": results_df["id"],
        "raw_text": results_df["raw_text"],
        "Negative": results_df["Negative"].astype(int),
        #"Active Voice": results_df["ActiveVoice"].astype(int),
        "Superlative": results_df["Superlative"].astype(int),
        "Subjective Language": results_df["Subjective"].astype(int),
        "Vague Pronouns": results_df["VaguePronouns"].astype(int),
        "Ambiguous Adverbs and adjectives": results_df["AmbiguousAd"].astype(int),
        "Comparative Phrases": results_df["CompPhrase"].astype(int),
        "Loopholes": results_df["Loopholes"].astype(int),
        "Open Ended Statements": results_df["OpenEnded"].astype(int),
        "Overall Conformace": results_df["overall_conformance"].astype(int),
    }
    ...
    filename = f"Output_Rule_Conformance.csv"
...
def batch_process_csv(
    ...
    if output_csv is None:
        ...
        output_csv = f"Output_Rule_Conformance_Details.csv"
    ...
    neg_clf, super_clf, subjective_clf, vagueP_clf, ambiguousAd_clf, compPhrase_clf, loophole_clf, openEnd_clf = load_classifiers()
    ...
    if disagreements:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        disagreements_file = f"Output_disagreements.csv"
