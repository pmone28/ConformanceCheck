...
def load_xlsx(path: Path) -> List[Dict]:
    ...
    for _, row in df.iterrows():
        text = ' '.join(str(cell) for cell in row if pd.notnull(cell)).strip()
        if text:
            blocks.append({"type": "row", "text": text})
    return blocks
...
def extract_requirements(blocks: List[Dict], source_file: str) -> List[Dict]:
    ...
    for block in blocks:
        ...
        # Row requirements (CSV/XLSX rows)
        if block["type"] == "row":
...
def export_json_database(rows: List[Dict], out_path: Path, source_file: str):
    ...
    print(f"Exported {len(output)} requirements to {out_path}")
...
def main():
    ...
    print("Extractor_LocalNLP_Ver2.py: multi-format input enabled (docx, pdf, xlsx, csv, txt)")
    if len(sys.argv) == 1:
        selected = input("Input file path: ").strip()
        if not selected:
    ...
    print(f"Exported requirement-likeness analysis to {req_likeness_path}")
